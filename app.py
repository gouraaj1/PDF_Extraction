...
DEFAULT_CONFIG = {
    ...
    "pdf_volume_path": os.getenv("PDF_VOLUME_PATH", "/Volumes/structured_data/extraction/pdf_files"),
    "results_table": os.getenv("RESULTS_TABLE", "pdf_extractions_normalized"),
    "log_table": os.getenv("LOG_TABLE", "pdf_extraction_run_log"),
    ...
    "mlflow_experiment": os.getenv("MLFLOW_EXPERIMENT", "/Users/ajai.goura@gmail.com/pdf-extraction-agent"),
    "chunk_size": int(os.getenv("CHUNK_SIZE", "10000")),
    "databricks_host": os.getenv("DATABRICKS_HOST", ""),
    "databricks_token": os.getenv("DATABRICKS_TOKEN", ""),
    "local_volume_fallback": os.getenv("LOCAL_VOLUME_FALLBACK", "./local_volume_store"),
    "enable_delta_write": os.getenv("ENABLE_DELTA_WRITE", "false").lower() == "true",
...
def best_effort_create_uc_objects(spark, config: dict[str, Any]):
    ...
    try:
        full_results = f"`{config['catalog']}`.`{config['schema']}`.`{config['results_table']}`"
        spark.sql(f"""
...
class VolumeManager:
    def __init__(self, config: dict[str, Any]):
        ...
        self.use_uc_volume = self.spark is not None
        ...
        for key, pattern in patterns.items():
            ...
            for m in matches:
                if isinstance(m, tuple):
                    vals.append(" ".join([str(x) for x in m if x]))
                else:
                    vals.append(str(m))
            results[key] = sorted(list({v.strip() for v in vals if v and v.strip()}))
        return results
            ...
            try:
                ...
                for key in EXTRACTION_FIELDS:
                    ...
                    if isinstance(vals, list):
                        aggregate[key].extend([str(v).strip() for v in vals if str(v).strip()])
            except Exception as exc:
                log.warning(f"LLM extraction chunk skipped: {exc}")

        for key in aggregate:
            aggregate[key] = sorted(list({v for v in aggregate[key] if v}))
        return aggregate


class ResultMerger:
    @staticmethod
    def merge(regex_results: dict[str, list[str]], llm_results: dict[str, list[str]]) -> dict[str, list[str]]:
        merged = {}
        for key in EXTRACTION_FIELDS:
            vals = regex_results.get(key, []) + llm_results.get(key, [])
            merged[key] = sorted(list({str(v).strip() for v in vals if str(v).strip()}))
        return merged
        ...
        for field in EXTRACTION_FIELDS:
            ...
            for value in values:
                ...
                if citation_base_url and citation_page:
                    if citation_base_url.startswith("file://"):
...
class DeltaWriter:
    ...
    def write_results(self, rows: list[dict[str, Any]]):
        if not self.spark or not self.config["enable_delta_write"] or not rows:
            return
        try:
            full_table = f"{self.config['catalog']}.{self.config['schema']}.{self.config['results_table']}"
    ...
    def write_log(
        ...
        if not self.spark or not self.config["enable_delta_write"]:
            return
        try:
            full_table = f"{self.config['catalog']}.{self.config['schema']}.{self.config['log_table']}"
            row = [{
                "run_id": run_id,
                "extraction_id": extraction_id,
                "file_name": file_name,
...
def render_clickable_table(df: pd.DataFrame):
    ...
    render_df["citation"] = render_df.apply(
        lambda row: (
            f'<a href="{row["citation_url"]}" target="_blank">Open PDF page {row["citation_page"]}</a>'
            if str(row.get("citation_url", "")).strip() and str(row.get("citation_page", "")).strip()
            else ""
        ),
        axis=1,
    )
...
def sidebar_config() -> dict[str, Any]:
    ...
    config = {
        "catalog": st.sidebar.text_input("Catalog", value=DEFAULT_CONFIG["catalog"]),
        "schema": st.sidebar.text_input("Schema", value=DEFAULT_CONFIG["schema"]),
        "volume": st.sidebar.text_input("Volume", value=DEFAULT_CONFIG["volume"]),
        "pdf_volume_path": st.sidebar.text_input("PDF Volume Path", value=DEFAULT_CONFIG["pdf_volume_path"]),
        "results_table": st.sidebar.text_input("Results Table", value=DEFAULT_CONFIG["results_table"]),
        "log_table": st.sidebar.text_input("Log Table", value=DEFAULT_CONFIG["log_table"]),
        "model": st.sidebar.text_input("Foundation Model", value=DEFAULT_CONFIG["model"]),
        "mlflow_tracking_uri": st.sidebar.text_input("MLflow Tracking URI", value=DEFAULT_CONFIG["mlflow_tracking_uri"]),
        "mlflow_experiment": st.sidebar.text_input("MLflow Experiment", value=DEFAULT_CONFIG["mlflow_experiment"]),
        "chunk_size": int(st.sidebar.number_input("Chunk Size", min_value=1000, max_value=50000, value=DEFAULT_CONFIG["chunk_size"], step=1000)),
        ...
        "enable_delta_write": st.sidebar.checkbox("Enable Delta Writes", value=DEFAULT_CONFIG["enable_delta_write"]),
        "enable_mlflow": st.sidebar.checkbox("Enable MLflow", value=DEFAULT_CONFIG["enable_mlflow"]),
    }
    return config


def main():
    st.set_page_config(page_title="PDF Extraction Agent POC", layout="wide")
    st.title("PDF Extraction Agent POC")
    st.write("Upload one or more PDFs, run extraction, and review results with citations.")

    config = sidebar_config()

    st.info(
        "POC note: In Databricks Free Edition, direct browser-openable Volume URLs may be limited. "
        "This app stores uploaded PDFs in a configured Volume when available, and falls back to local storage otherwise. "
        "Citations are clickable using the best practical path available for the running environment."
    )
    ...
    if run_button:
        ...
        if config["enable_mlflow"]:
            try:
                mlflow_run = mlflow.start_run(run_name=f"streamlit_pdf_extraction_{int(time.time())}")
                mlflow.log_params({
        ...
        try:
            for idx, uploaded_file in enumerate(uploaded_files, start=1):
                ...
                if result["status"] == "success":
                    ...
                    failed_files.append((result["file_name"], result.get("error", "Unknown error")))
            ...
            if success_files and not failed_files:
                st.success(f"Agent run completed successfully for {len(success_files)} file(s).")
            elif success_files and failed_files:
                st.warning(
                    f"Agent run completed with partial success. Success: {len(success_files)}, Failed: {len(failed_files)}"
                )
            else:
            ...
            if all_rows:
                ...
                if config["enable_mlflow"] and mlflow.active_run():
                    try:
                        mlflow.log_metric("processed_file_count", len(uploaded_files))
                        mlflow.log_metric("success_file_count", len(success_files))
                        mlflow.log_metric("failed_file_count", len(failed_files))
                        for field in EXTRACTION_FIELDS:
                            mlflow.log_metric(
                                f"{field}_count",
